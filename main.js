...
carrito.addEventListener('mouseover', mostrarCarrito)
...
function mostrarCarrito() {
    ...
    // Mostrar el contenido del carrito en el elemento "revelar"
    revelar.innerHTML = productosCarrito.map(articulo => {
        return `
            <div>
                <img class="w-[50px] h-[50px]" src="${articulo.img}"/>
                <p>${articulo.nombre}</p>
                <p>${articulo.precio}</p>
                <p>${articulo.cantidad}</p>
            </div>
        `;
    }).join(""); // Asegúrate de unir los elementos del array en una cadena

    // Cambia el estilo del elemento "revelar" a "block"
    revelar.style.display = "block";
    revelar.style.zIndex = "100"
}

carrito.addEventListener('mouseout', quitarCarrito)
...
function agregarCarrito (e) {
    productosCarrito.push({
        ...
        descripcion: e.target.parentElement.parentElement.querySelector('p:nth-child(4)').innerText,
        cantidad: e.target.parentElement.parentElement.querySelector('p:nth-child(5)').innerText
    });
...
// descripcion.addEventListener('click', mostrarDescripcion())

// const mostrarDescripcion = () => {
//     productos.map (articulo => {
//         const { img, nombre, precio, descripcion, cantidad } = articulo;
...
//                 <button class="w-[50%] h-[30px] bg-slate-500 rounded-lg  text-xl font-bold text-white descripcion" > Cerrar</button>
//             </div>
//             `;
...
function articulosCuadrilla () {
    productos.map( articulo => {
        ...
        cuadrilla.innerHTML = `
            ...
            <div class="w-[100%] flex justify-center items-center mt-[10px] gap-8">
                <button onClick="agregarCarrito(event)" class="w-[50%] h-[30px] bg-slate-500 rounded-lg text-xl font-bold text-white" > Agregar al carrito </button> 
                <button class="w-[50%] h-[30px] bg-slate-500 rounded-lg  text-xl font-bold text-white descripcion" > Descripción</button>
            </div>
            `;
